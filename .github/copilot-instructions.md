# Copilot Instructions — MicroPython STM32 Development Project
...
Search the web to determine whether an official MicroPython port exists for the requested board:

1. Check `micropython.org/download/<BOARD_NAME>` (e.g., `NUCLEO_H563ZI`).
2. Check the MicroPython GitHub repo (`micropython/micropython`) for a board definition under `ports/stm32/boards/<BOARD_NAME>/`.
3. Look for community ports or forks if no official port is found.
...
Determine the board's family prefix from the MCU family name in lowercase:
- `NUCLEO_G0B1RE` -> family `G0` -> prefix `g0_`
- `NUCLEO_G474RE` -> family `G4` -> prefix `g4_`
- `NUCLEO_H563ZI` -> family `H5` -> prefix `h5_`
- `NUCLEO_F446RE` -> family `F4` -> prefix `f4_`
- General rule: take the first letter + digit of the MCU family, lowercase it, append `_`.
...
## Code Style for MicroPython Scripts

- Use `machine` module as primary API, fall back to `pyb` when needed.
- Include try/except for imports to handle API differences across board families.
- Use board-level pin names from `<PREFIX>_board_config.json` pin_map.
- Print `PASS`/`FAIL`/`SKIP` markers in test scripts for automated parsing.
- Keep scripts minimal — no unnecessary abstractions or comments.
