...
Set the real-time clock to a known date/time, wait, and verify it advances correctly.

## Notes

The STM32G0B1RE has an on-chip RTC with calendar functionality. An external 32.768 kHz crystal (LSE) is populated on the NUCLEO board, providing accurate timekeeping.

## Test Script

Generate `tests/<PREFIX>_test_rtc.py`:

```python
import time

# Test 1: RTC init via pyb module
try:
    from pyb import RTC
    rtc = RTC()
...
try:
    if use_pyb:
        ...
        # machine.RTC may use different format
        rtc.datetime((2026, 5, 6, 1, 14, 30, 0, 0))
...
```bash
python scripts/pyboard.py --device <COM_PORT> -f cp tests/<PREFIX>_test_rtc.py :
python scripts/pyboard.py --device <COM_PORT> tests/<PREFIX>_test_rtc.py
```

## Verification

- **Pass**: Output contains `RTC_TEST_COMPLETE` and `PASS: RTC advanced`
- **Fail**: If `pyb.RTC` is not available, `machine.RTC` should work as fallback
- **Note**: The 3-second sleep is intentional — it proves the RTC oscillator is running

## Result Schema

```json
{ "module": "rtc", "status": "PASS|FAIL", "attempts": N, "details": "..." }
```
